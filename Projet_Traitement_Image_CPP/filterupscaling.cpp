...
filterupscaling::filterupscaling()
{

}

// Sets the new dimensions for the rescaling operation based on a string input
bool filterupscaling::setParameters(std::string const & _line)
{
    ...
    if(m_scalor < 1.f){ // Error handling
        throw std::invalid_argument("upscaling: Scale factor must be >= 1.f");
        return false;
    }
...
Image * filterupscaling::create(Image const * const _in)
{
...
void filterupscaling::compute(unsigned int const w, unsigned int const h, Image const * const _in, Image * const _out) const
{
    // Original dimensions
